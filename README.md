...
```
- **Discard các file chưa commit ở local**
```
...
```
git for-each-ref --sort=-committerdate refs/heads/ | headTar 
```
cả project, ngoại trừ thư mục .git
```
cd ..
tar cJf project.tar.xz project/ --exclude-vcsTar
```
- **Tất cả các file bị thay đổi ở local**
```
